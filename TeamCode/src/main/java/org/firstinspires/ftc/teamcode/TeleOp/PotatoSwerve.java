...
public class PotatoSwerve extends LinearOpMode {
    ...
    /* ===================== ADJUSTER SERVO (GAMEPAD2 LEFT STICK Y) ===================== */
    // You specified: 0.0 = all the way up, 0.75 = at the top (as written)
    // We'll respect your bounds exactly and let the stick "trim" within [0.0, 0.75].
    final double ADJUSTER_MIN = 0.0;
    final double ADJUSTER_MAX = 0.75;
    final double ADJUSTER_STEP = 0.01;
    final double ADJUSTER_DEADBAND = 0.05;
    private double adjusterPos = ADJUSTER_MIN; // start "all the way up"
    ...
    /* ===================== TRIGGER SERVO ===================== */
    // 0.0 = launched state, 0.225 = down/reset
    ...
    // Longer hold specifically for launch sequence
    final long LAUNCH_TRIGGER_HOLD_MS = 400;

    // Ensure we give the trigger time to actually get back down before feeding the next ball
    ...
    /* ===================== LAUNCH BEHAVIOR ===================== */
    // If nothing reaches center within 1 second, fire anyway.
    final long FEED_TIMEOUT_MS = 1000;

    /* ===================== DISTANCE BALL DETECTION ===================== */
    ...
    // Rising-edge detection for "new center ball"
    ...
    // Launch feed power (inward)
    final double LAUNCH_INTAKE_POWER = 0.76;

    // Used to implement FEED timeout
    ...
    public void runOpMode() {
        ...
        telemetry.addLine("PotatoSwerve ready (Distance-based ball detect)");
        telemetry.addLine("Drive slow: GP1 RB | Launch: A | Abort: B | Intake toggle: RT | Intake mode: dpad_left");
        telemetry.addLine("Turret: GP2 RSX | Adjuster: GP2 LSY");
        telemetry.update();
        ...
        while (opModeIsActive()) {
            ...
            /* ===================== ADJUSTER (GAMEPAD2 LEFT STICK Y) ===================== */
            // Using incremental trim like turret. Push stick up/down to move between [0.0, 0.75].
            double adjInput = -gamepad2.left_stick_y; // up = positive
            if (Math.abs(adjInput) < ADJUSTER_DEADBAND) adjInput = 0;

            adjusterPos += adjInput * ADJUSTER_STEP;
            adjusterPos = clamp(adjusterPos, ADJUSTER_MIN, ADJUSTER_MAX);
            adjuster.setPosition(adjusterPos);
            ...
            telemetry.addData("AdjusterPos", "%.3f", adjusterPos);
            telemetry.addData("TriggerPos", "%.3f", trigger.getPosition());
            telemetry.addData("Dist F/C/B (cm)", "%.2f / %.2f / %.2f", fCm, cCm, bCm);
